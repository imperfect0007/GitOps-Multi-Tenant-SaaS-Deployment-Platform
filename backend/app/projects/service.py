...
def _create_argocd_application(name: str, namespace: str):
    """Create or update ArgoCD Application. Gracefully skips when cluster is unreachable."""
    try:
        try:
        ...
        logger.warning("Could not create ArgoCD application for %s/%s (cluster may be down): %s. Manifests are in GitOps repo; ArgoCD will sync when cluster is available.", namespace, name, e)
